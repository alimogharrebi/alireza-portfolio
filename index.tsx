...
// --- Constants ---
const TEACHING_RECORDS: TeachingRecord[] = [
...
const App = () => {
  return (
    <div className="min-h-screen selection:bg-cyan-500/30">
      ...
      <header className="relative pt-48 pb-32 overflow-hidden">
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-4xl">
            ...
            <h1 className="text-6xl md:text-8xl font-black mb-8 leading-[1.15] animate-fade-in-up" style={{ animationDelay: '100ms' }}>
              تحلیل‌گر <br />
              <span className="bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-500 bg-clip-text text-transparent">ارشد داده</span>
            </h1>
            <p className="text-xl md:text-2xl text-slate-400 mb-12 max-w-3xl leading-relaxed animate-fade-in-up" style={{ animationDelay: '200ms' }}>
            ...
            <div className="flex flex-wrap gap-6 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
              <div className="flex items-center gap-3 bg-slate-900/50 backdrop-blur-sm border border-slate-800 px-6 py-3 rounded-2xl text-slate-300 hover:border-slate-700 transition-all">
      ...
      {/* About / Education */}
      <section id="about" className="py-24 bg-slate-950/50 relative border-y border-slate-900/50">
        <div className="container mx-auto px-6">
          <SectionHeading icon={GraduationCap} id="education">تحصیلات و مدارک</SectionHeading>
          <div className="grid md:grid-cols-2 gap-8">
            <div className="group relative p-8 rounded-3xl bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800 hover:border-cyan-500/50 transition-all duration-500">
            ...
            <div className="group relative p-8 rounded-3xl bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800 hover:border-blue-500/50 transition-all duration-500">
              <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity">
                <Briefcase size={100} />
              </div>
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {SKILLS.map((group, idx) => (
              <div key={idx} className="flex flex-col p-10 rounded-[2.5rem] bg-slate-900/40 border border-slate-800 hover:bg-slate-900/60 hover:-translate-y-2 transition-all duration-500 group">
                <div className="mb-8 p-4 rounded-2xl bg-slate-950/50 border border-slate-800 inline-block w-fit group-hover:scale-110 transition-transform">
                  {group.icon}
                </div>
                <h3 className="text-xl font-bold mb-8 text-white">{group.category}</h3>
                <ul className="space-y-4 flex-1">
                  {group.skills.map((skill, sIdx) => (
                    <li key={sIdx} className="flex items-center gap-3 text-sm text-slate-400 group-hover:text-slate-300 transition-colors">
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {TEACHING_TOPICS.map((topic, idx) => (
              <div key={idx} className="flex items-center gap-5 p-6 rounded-[1.5rem] bg-slate-900/80 border border-slate-800/50 hover:bg-slate-800 hover:border-cyan-500/30 transition-all group">
                <div className="w-12 h-12 rounded-xl bg-cyan-500/10 flex items-center justify-center text-cyan-400 font-black text-xl group-hover:bg-cyan-500 group-hover:text-white transition-all">
                  {idx + 1}
                </div>
                <span className="font-bold text-slate-200 group-hover:text-white transition-colors">{topic}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="relative">
            <div className="overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-900/30 backdrop-blur-md overflow-x-auto">
                    ...
                    <tr key={record.id} className="hover:bg-cyan-500/5 transition-all group">
                      <td className="p-8">
                        <div className="flex items-center gap-3">
                          <CheckCircle2 size={18} className="text-cyan-500 opacity-0 group-hover:opacity-100 transition-opacity" />
              ...
              <div className="p-10 bg-slate-900/40 text-center">
                <p className="text-slate-400 italic">و بیش از ۲۰۰ دوره تخصصی دیگر در شرکت‌های تابعه نفت، گاز، پتروشیمی و صنایع معدنی...</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Projects */}
      <section id="projects" className="py-32 bg-slate-950/80 relative">
        <div className="absolute top-1/2 left-0 -translate-y-1/2 w-64 h-64 bg-blue-600/5 blur-[100px] -z-10"></div>
        <div className="container mx-auto px-6">
          <SectionHeading icon={Layout} id="projects-section">پروژه‌های شاخص</SectionHeading>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
            {PROJECTS.map((project) => (
              <div key={project.id} className="group flex flex-col p-10 rounded-[3rem] bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 hover:border-blue-500/40 transition-all duration-700">
                ...
                <h3 className="text-2xl font-black mb-5 text-white group-hover:text-blue-400 transition-colors leading-snug">{project.title}</h3>
      ...
      {/* Contact Footer */}
      <footer id="contact" className="py-32 border-t border-slate-900/50 bg-slate-950 relative overflow-hidden">
        <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-cyan-600/5 blur-[120px] rounded-full"></div>
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl text-slate-400 mb-16 leading-relaxed">
              برای مشاوره در حوزه هوش تجاری، تحلیل داده و یا هماهنگی دوره‌های آموزشی می‌توانید از طریق راه‌های زیر با من در ارتباط باشید.
            </p>
            
            <div className="flex flex-wrap justify-center gap-8 mb-20">
              <a href="mailto:example@email.com" className="flex items-center gap-4 px-8 py-5 rounded-3xl bg-slate-900 border border-slate-800 hover:border-cyan-500/50 hover:bg-slate-800 transition-all group">
              ...
              <a href="#" className="flex items-center gap-4 px-8 py-5 rounded-3xl bg-slate-900 border border-slate-800 hover:border-blue-500/50 hover:bg-slate-800 transition-all group">
              ...
              <a href="#" className="flex items-center gap-4 px-8 py-5 rounded-3xl bg-slate-900 border border-slate-800 hover:border-slate-500/50 hover:bg-slate-800 transition-all group">
                <Github className="text-white group-hover:scale-110 transition-transform" />
                <span className="font-bold text-white">گیت‌هاب</span>
              </a>
            </div>

            <div className="pt-20 border-t border-slate-900 flex flex-col md:flex-row justify-between items-center gap-6 opacity-60">
              <p className="text-sm font-medium">
                تمامی حقوق محفوظ است © {new Date().getFullYear()} - علیرضا مقربی
              </p>
              <div className="flex gap-8 text-sm">
                <a href="#about" className="hover:text-cyan-400 transition-colors">درباره</a>
                <a href="#skills" className="hover:text-cyan-400 transition-colors">مهارت‌ها</a>
                <a href="#" className="hover:text-cyan-400 transition-colors">رزومه (PDF)</a>
              </div>
            </div>
          </div>
        </div>
